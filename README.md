...
- 🐛 **Sửa lỗi trang dịch bị cắt giữa chừng:** Gemini luôn gửi `maxOutputTokens: 65535`, còn nhánh OpenAI Compatible chỉ gửi `max_tokens` khi người dùng tự nhập — mà giao diện không có ô nhập, nên tham số luôn bị bỏ qua và endpoint áp mặc định của nó (nhiều gateway để 1024). Kết quả: trang chỉ dịch được đoạn đầu rồi dừng. Nay luôn gửi **65535 token** đúng như Gemini.
- 🐛 **Không còn âm thầm nhận nửa trang:** App trước đây không đọc `finish_reason`. Máy chủ đã báo rõ `length` (bị cắt) nhưng app vẫn ghi phần dang dở vào kết quả như thể hoàn chỉnh. Nay `finish_reason` được đọc ở cả hai dialect (Gemini lẫn OpenAI) và cả hai luồng (SSE lẫn JSON đệm); gặp `length` hoặc `content_filter` thì báo lỗi rõ ràng kèm số token đã sinh.
