...
class HistoryManager {
   ...
   */
  /** Trả về runId của mục vừa xoá (nếu có) để nơi gọi xoá nốt checkpoint. */
   ...
   * Clear all history
   */
  /** Trả về danh sách runId đã xoá để nơi gọi xoá nốt checkpoint tương ứng. */
  async clearAll() {
    await this._loadHistory();
    const runIds = this.historyCache.map(h => h.runId).filter(Boolean);
    this.historyCache = [];
    await this._saveHistory();
    return runIds;
  }
