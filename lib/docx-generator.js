...
class DocxGenerator {
  ...
  async generate(pageResults, outputPath) {
    ...
    for (let idx = 0; idx < pageResults.length; idx++) {
      ...
      // Reconstruct bilingual sections if missing but bilingual mode is enabled
      let pageBilingualSections = pageResults[idx].bilingualSections;
      if (!pageBilingualSections && (this.settings.bilingual || this.settings.translateMode === 'bilingual') && markdown) {
        pageBilingualSections = reconstructBilingualSections(markdown);
      }

      // Bilingual mode: interleaved layout — original paragraph → translation paragraph
      if (this.settings.translateLanguage && (this.settings.translateMode === 'bilingual' || this.settings.bilingual) && pageBilingualSections && pageBilingualSections.length > 0) {
        for (const sec of pageBilingualSections) {
