...
class OpenAICompatibleProvider extends BaseProvider {
   ...
   * @param {object} options   { supportsVision, maxTokens, pricing: { input, output } }
   */
  constructor(baseUrl = '', apiKey = '', model = '', options = {}) {
    super(model || '');
    this.baseUrl = OpenAICompatibleProvider.normalizeBaseUrl(baseUrl);
    this.apiKey = apiKey || '';
    this.supportsVision = options.supportsVision !== false; // default: assume vision-capable
  ...
  _buildImageRequestBody(prompt, base64Image) {
    if (!this.supportsVision) {
      throw new Error(
        `Model "${this.model}" được đánh dấu là không hỗ trợ ảnh (vision). ` +
        `Hãy bật "Model hỗ trợ ảnh" trong Cài đặt hoặc chọn model vision khác để OCR file PDF.`
      );
    }
