...
class BaseProvider {
    ...
    return {
      ...
      finish() {
        ...
        return {
          text: fullText,
          usageStats,
          diagnostics: { rawSample, payloadCount, parseErrors, source: 'sse' },
        };
      },
    };
  }
  ...
  async processImageStream(imageBuffer, prompt, onChunk, { signal } = {}) {
    ...
    return this._assertNonEmpty(
      await this._handleStreamResponse(response, onChunk),
      'trích xuất nội dung từ ảnh trang tài liệu'
    );
  }
  ...
  async translateTextStream(text, targetLang, sourceLang = 'auto', bilingual = false, onChunk, { signal } = {}) {
    ...
    const { text: raw, usageStats } = this._assertNonEmpty(
      await this._handleStreamResponse(response, onChunk),
      'dịch văn bản'
    );
