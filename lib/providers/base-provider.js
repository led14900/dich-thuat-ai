...
class BaseProvider {
  ...
  // Hard ceiling on the encoded image. 0 = only the edge limit applies.
  ...
  async _prepareImage(imageBuffer) {
    ...
    try {
      ...
      for (const edge of edges) {
        ...
        if (!maxBytes || candidate.buffer.length <= maxBytes) return candidate;
      }
  ...
  async processImageStream(imageBuffer, prompt, onChunk, { signal } = {}) {
    ...
    const result = this._assertNonEmpty(
      this._assertNotTruncated(await this._handleStreamResponse(response, onChunk), what),
      what
    );
  ...
  async translateTextStream(text, targetLang, sourceLang = 'auto', bilingual = false, onChunk, { signal } = {}) {
    ...
    const { text: raw, usageStats, truncationWarning } = this._assertNonEmpty(
      this._assertNotTruncated(await this._handleStreamResponse(response, onChunk), 'dịch văn bản'),
      'dịch văn bản'
    );
