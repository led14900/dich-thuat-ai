...
class RunCheckpointManager {
  ...
  clear(runId) {
    try { fs.unlinkSync(this._metaFileFor(runId)); } catch { /* may not exist */ }
    try {
  ...
  _metaFileFor(runId) {
    return this._fileFor(runId).replace(/\.jsonl$/, '.meta.json');
  }

  /**
   * Remember which document a run belongs to, so "Chạy tiếp" can tell the user
   * what they are resuming instead of showing a bare run id.
   */
  saveMeta(runId, meta) {
    try {
      this._ensureDir();
      // Gộp với meta đang có, để nơi gọi chỉ cần sửa một trường (ví dụ đánh dấu
      // đã ghi vào Lịch sử) mà không phải đọc rồi ghi lại toàn bộ.
      const merged = { ...(this.readMeta(runId) || {}), ...meta };
      fs.writeFileSync(this._metaFileFor(runId), JSON.stringify(merged, null, 2));
      return true;
    } catch (err) {
      console.error('[checkpoint] không lưu được meta:', err.message);
      return false;
    }
  }
  ...
  readMeta(runId) {
    try {
      return JSON.parse(fs.readFileSync(this._metaFileFor(runId), 'utf8'));
    } catch {
      return null;
    }
  }
  ...
  /** Most recent unfinished run for a given document, if any. */
  findForFile(filePath) {
    if (!filePath) return null;
    return this.list().find(r => r.meta?.filePath === filePath) || null;
  }
